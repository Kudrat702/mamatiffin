...
import { initNotifications, getStoredEndpoint } from '../services/notificationService';
import {
  ArrowLeft,
  IndianRupee,
  Clock,
...
interface PaymentData {
  ...
  totalAmount: number;
  duration: number;
  subscriptionType: 'monthly' | 'trial' | 'weekly';  // ✅ Added 'weekly'
  deliveryTime?: string;
  description?: string;
  imageUrl?: string;
...
const PaymentPage: React.FC = () => {
  ...
  const [pushEndpoint, setPushEndpoint] = useState<string | null>(getStoredEndpoint);
  ...
  useEffect(() => {
    initNotifications('customer').then((endpoint) => {
      if (endpoint) setPushEndpoint(endpoint);
    });
  }, []);
  ...
  const getSubscriptionLabel = (): string => {
    if (!paymentData) return 'Plan';
    if (paymentData.subscriptionType === 'trial') return '1 Day Trial';
    if (paymentData.subscriptionType === 'weekly') return '7 Days Weekly Plan';
    return 'Monthly Plan';
  };

  // ✅ Helper function to get price label
  const getPriceLabel = (): string => {
    if (!paymentData) return 'Price';
    if (paymentData.subscriptionType === 'trial') return 'Trial Price';
    if (paymentData.subscriptionType === 'weekly') return 'Weekly Price';
  ...
  const createPaymentOrder = async (): Promise<PaymentOrderResponse['data']> => {
    try {
      ...
      console.log('Subscription Type:', paymentData.subscriptionType);
    ...
    try {
      const response = await fetch(apiEndpoints.verifyPayment, {
        ...
        body: JSON.stringify({
          ...
          dbOrderId: dbOrderId,
          customerSubscriptionEndpoint: pushEndpoint ?? undefined,
        })
      });
        ...
        handler: async (response: RazorpayResponse) => {
          try {
            ...
            if (verificationResult.success) {
              ...
              setPendingVerification({ paymentResponse: response, dbOrderId: orderData.dbOrderId });
              setError(verificationResult.message || 'Order confirmation failed. Use "Retry Confirmation" below.');
              setLoading(false);
            }
          } catch {
            // Network error after payment was processed — do NOT show "payment failed"
            setPendingVerification({ paymentResponse: response, dbOrderId: orderData.dbOrderId });
            setError('Connection lost after payment. Click "Retry Confirmation" to confirm your order.');
            setLoading(false);
          }
        },
        modal: {
  ...
  const retryVerification = async () => {
    ...
    try {
      const result = await verifyPayment(
        pendingVerification.paymentResponse,
        pendingVerification.dbOrderId
      );
      if (result.success) {
        setPendingVerification(null);
        navigate(`/payment/success?orderId=${pendingVerification.dbOrderId}&paymentId=${pendingVerification.paymentResponse.razorpay_payment_id}`);
      } else {
        setError(result.message || 'Confirmation still failing. Please contact support.');
      }
    } catch {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 flex items-center justify-center p-4">
        <div className="text-center max-w-md bg-white p-8 rounded-2xl shadow-2xl">
          ...
          <p className="text-gray-600 mb-1">Your payment was processed by Razorpay but we lost the connection before confirming your order.</p>
          <p className="text-sm text-gray-500 mb-1 mt-3">
            Payment ID: <span className="font-mono font-semibold text-gray-700">{pendingVerification.paymentResponse.razorpay_payment_id}</span>
          </p>
          <p className="text-xs text-gray-400 mb-6">Save this ID for support reference.</p>
          {error && (
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className="bg-[#328c81] hover:bg-[#2a7568] disabled:bg-gray-400 text-white font-bold py-3 px-6 rounded-xl transition-all duration-200 flex items-center justify-center gap-2"
            >
              {isVerifying ? <><Loader className="w-4 h-4 animate-spin" /> Confirming...</> : <><CheckCircle className="w-4 h-4" /> Retry Confirmation</>}
            </button>
            <a
              href={`mailto:support@mamatiffin.com?subject=Payment Confirmation Needed&body=Payment ID: ${pendingVerification.paymentResponse.razorpay_payment_id}`}
              className="text-sm text-[#328c81] hover:underline"
            >
            ...
            {paymentData && (
              <>
                <div className="relative h-56 rounded-xl overflow-hidden mb-6 group">
                  ...
                  {/* ✅ NEW: Subscription Type Badge */}
                  <div className="absolute top-3 right-3">
                    <span className={`px-4 py-2 rounded-full text-sm font-bold shadow-lg ${
                      paymentData.subscriptionType === 'trial' 
                        ? 'bg-orange-500 text-white' 
                        : paymentData.subscriptionType === 'weekly'
                    ...
                    <div className="bg-gradient-to-r from-[#328c81]/10 to-[#2a7568]/10 p-4 rounded-xl border border-[#328c81]/20">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {/* ✅ UPDATED: Show correct duration based on subscription type */}
