...
//   const [imagePublicId, setImagePublicId] = useState<string>(''); // NEW: Cloudinary publicId ✅
...
//         // NEW: Include imagePublicId when using existing image ✅
//         if (imagePublicId) {
//           formDataToSend.append('imagePublicId', imagePublicId);
//           if (isDev) console.log('Including existing imagePublicId:', imagePublicId);
//         }
...
//         // NEW: Store imagePublicId from response ✅
//         if (responseData.data?.imagePublicId) {
//           setImagePublicId(responseData.data.imagePublicId);
//           if (isDev) console.log('Received imagePublicId:', responseData.data.imagePublicId);
//         }
...
//           setImagePreview('');
//           setImagePublicId(''); // NEW: Reset imagePublicId ✅
//         }
...
//       imagePublicId: imagePublicId || 'null' // NEW: Log imagePublicId ✅
//     });
//   }
...
//                   <div>ImagePublicId: {imagePublicId || 'null'}</div>
//                 </div>
//               )}
...
const MenuForm: React.FC<MenuFormProps> = ({ menu, onClose, onDelete }) => {
  ...
  useEffect(() => {
    if (menu) {
      ...
      // ✅ UPDATED: NO auto-calculation - load exact values from database
      const newFormData = {
        category: menu.category || 'veg',
        menuType: menu.menuType || '',
        title: menu.title || `${menu.category} ${menu.menuType} Menu`,
        ...
        deliveryTime: menu.deliveryTime || '',
        priceMonthly: menu.priceMonthly ? menu.priceMonthly.toString() : '',
        priceWeekly: menu.priceWeekly ? menu.priceWeekly.toString() : '',  // ✅ NO fallback
        priceTrial: menu.priceTrial ? menu.priceTrial.toString() : '',     // ✅ NO fallback
        weeklyMenu: weeklyMenuArray
      };
  ...
  // ✅ UPDATED: NO auto-calculation - all 3 prices are independent
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const { name, value } = e.target;

    if (!name) return;

    // ✅ All 3 price fields handled separately, NO auto-calculation
  ...
  const validateForm = (): string | null => {
    ...
    if (isNaN(weeklyPrice) || weeklyPrice <= 0) {
      return 'Please enter a valid weekly price greater than 0';
    }
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                {/* ✅ Weekly Price - Manual Entry Only */}
                <div>
                  <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Enter exact weekly price for customers
                  </p>
                </div>
