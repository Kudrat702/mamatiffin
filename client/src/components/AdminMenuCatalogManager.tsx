...
const imageUploadUtils = {
  ...
  deleteImageFromCloudinary: async (publicId: string): Promise<void> => {
    try {
      // Safely resolve delete endpoint: prefer apiEndpoints.deleteImage if present,
      // otherwise fall back to uploadImage (backend may expose a delete route at the same base).
      const endpoints = apiEndpoints as { deleteImage?: string; uploadImage?: string };
      const deleteEndpoint = endpoints.deleteImage ?? endpoints.uploadImage;
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 text-lg mt-2">Manage restaurant menu items with Cloudinary storage</p>
            </div>
            <div className="flex items-center space-x-4">
              <div className="flex items-center text-gray-600 bg-gray-100 px-4 py-2 rounded-lg">
                <ChefHat className="w-5 h-5 mr-2" />
