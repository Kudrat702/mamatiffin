...
import AdminRoom from './AdminRoom'; // ✅ ADDED: Room management component
...
// ✅ UPDATED: Added 'rooms' to AdminView type
type AdminView =
  | 'dashboard'
  | 'slider-management'
  | 'menu-details'
...
const convertAPIMenuToMenu = (apiMenu: MenuDetailsFromAPI): Menu => {
  ...
  return {
    ...
    priceWeekly: apiMenu.priceWeekly !== undefined && apiMenu.priceWeekly !== null 
  ? Number(apiMenu.priceWeekly) 
  : 0,
...
const AdminDashboard: React.FC = () => {
  ...
  const [pushEnabled, setPushEnabled] = useState(() => !!getStoredEndpoint());

  const handleEnableNotifications = async () => {
    const endpoint = await initNotifications('admin');
    if (endpoint) setPushEnabled(true);
  };

  // Location Count for Sidebar Badge
  const [locationCount, setLocationCount] = useState(0);

  // ✅ ADDED: Room Count for Sidebar Badge
  ...
  const menuItems = [
    ...
    // ✅ ADDED: Rooms menu item (after locations)
    {
      id: 'rooms' as AdminView,
      label: 'Rooms',
      icon: Bed,
      description: 'Lodge & Hostel Listings',
  ...
  // ✅ ADDED: Fetch room count for sidebar badge
  const fetchRoomCount = useCallback(async () => {
    try {
      const token = authContext?.token || localStorage.getItem('ADMIN_TOKEN') || '';
      const response = await fetch(`${apiEndpoints.adminRooms}?limit=1`, {
        headers: { Authorization: `Bearer ${token}` },
        credentials: 'include',
      });
      const data = await response.json();
      if (data.success) {
        setRoomCount(data.total || 0);
      }
    } catch (error) {
  ...
  // ✅ UPDATED: Added 'rooms' page title
  ...
  // ✅ UPDATED: Added fetchRoomCount to interval
  useEffect(() => {
    fetchMessageStats();
    fetchSystemStats();
    fetchLocationCount();
    fetchRoomCount(); // ✅ ADDED
  ...
  }, [fetchMessageStats, fetchSystemStats, fetchLocationCount, fetchRoomCount]); // ✅ Updated deps
  ...
  <p className="text-gray-500 mt-1">
    Manage Orders, Menu Aur Delivery Areas
        ...
        {/* ✅ UPDATED: Changed grid to 6 columns (added Rooms button) */}
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          ...
          {/* ✅ ADDED: Rooms quick action button */}
          <Button
            onClick={() => setCurrentView('rooms')}
            className="relative flex h-auto flex-col items-center rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 p-6 text-white shadow-sm transition-all duration-200 hover:scale-105 hover:from-orange-600 hover:to-amber-600 hover:shadow-md"
          >
            <Bed className="mb-3 h-10 w-10" />
            <span className="text-lg font-semibold">Rooms</span>
            <span className="mt-1 text-xs opacity-90">Lodge Listings</span>
            {roomCount > 0 && (
              <span className="absolute -top-2 -right-2 rounded-full bg-white px-2 py-1 text-xs font-bold text-orange-600">
                {roomCount}
              </span>
            )}
          </Button>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        {/* ✅ UPDATED: Combined Location + Room overview */}
        <div className="rounded-xl border border-gray-100 bg-white p-8 shadow-sm">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-900">Locations & Rooms</h2>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <button
              onClick={() => setCurrentView('locations')}
              className="rounded-lg bg-emerald-50 p-4 text-center hover:bg-emerald-100 transition-colors"
            >
              <MapPin className="mx-auto mb-2 h-6 w-6 text-emerald-600" />
              <div className="text-3xl font-bold text-emerald-600">{locationCount}</div>
              <div className="text-xs text-gray-600 mt-1">Delivery Areas</div>
            </button>
            <button
              onClick={() => setCurrentView('rooms')}
              className="rounded-lg bg-orange-50 p-4 text-center hover:bg-orange-100 transition-colors"
            >
              <Bed className="mx-auto mb-2 h-6 w-6 text-orange-600" />
              <div className="text-3xl font-bold text-orange-600">{roomCount}</div>
              <div className="text-xs text-gray-600 mt-1">Lodge Listings</div>
            </button>
          </div>
          <div className="mt-4 text-center">
            <p className="text-xs text-gray-500">
              {locationCount === 0 && roomCount === 0
                ? 'Add your first location and room'
                : 'Click to manage'}
            </p>
          </div>
        </div>
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'rooms':
        return (
          <div className="min-h-[80vh] rounded-xl border border-gray-100 bg-white shadow-sm">
            <AdminRoom onNotification={showDashboardNotification} />
          </div>
        );
          ...
          <div className="flex h-16 items-center justify-between px-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                {!pushEnabled && (
                  <span className="absolute -top-1 -right-1 h-3 w-3 rounded-full bg-orange-400"></span>
                )}
                {showNotificationBadge && pushEnabled && (
