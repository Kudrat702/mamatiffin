  ...
  const fetchMenus = async (): Promise<void> => {
    try {
      ...
      if (nonVegData.success && nonVegData.data && nonVegData.data.length > 0) {
        ...
        console.log(
          '🔍 SAMPLE NON-VEG MENU weeklyMenu type:',
          typeof nonVegData.data[0]?.weeklyMenu
        );
        console.log(
          '🔍 SAMPLE NON-VEG MENU weeklyMenu length:',
          nonVegData.data[0]?.weeklyMenu?.length
        );
      }
            ...
            {filteredMenus.map(menu => (
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  // UPDATED CODE (Consistent with Cloudinary):
                  <img
                    src={apiEndpoints.getImageUrl(menu.imageUrl || '')} // Simply use getImageUrl - it handles everything
