...
// controllers/menuController.ts - CLOUDINARY INTEGRATED + priceWeekly BUG FIXED
import { Request, Response } from 'express';
import { MenuDetails, IWeeklyMenuDay } from '../models/menuDetails';
import { uploadToCloudinary, deleteFromCloudinary } from '../utils/cloudinaryUpload';
...
// Handle image URL generation for Cloudinary or local
const handleImageUrl = async (req: Request): Promise<{ imageUrl: string; imagePublicId?: string }> => {
  // Priority 1: New file upload
  if (req.file) {
    console.log('📤 Processing menu image upload...');
...
// ✅ Create or update VEG menu — priceWeekly BUG FIXED
export const createOrUpdateVegMenu = async (req: Request, res: Response): Promise<void> => {
  try {
    console.log('🚀 VEG Menu Controller called');
    console.log('🌐 Using Cloudinary:', useCloudinary ? 'YES' : 'NO');
    ...
    // ✅ FIX: priceWeekly ab destructuring mein hai
    const {
      menuType,
      title,
      description,
      deliveryTime,
      priceMonthly,
      priceWeekly,   // ✅ ADDED — pehle yahan nahi tha, isliye 0 save hota tha
      priceTrial,
      weeklyMenu
    } = req.body;
    ...
    // ✅ FIX: priceWeekly ab destructured variable se aata hai, req.body se nahi
    const menuData: any = {
      category: 'veg' as const,
      menuType,
      title,
      ...
      imageUrl: imageData.imageUrl,
      deliveryTime,
      priceMonthly: parseFloat(priceMonthly?.toString() || '0'),
      priceWeekly: parseFloat(priceWeekly?.toString() || '0'),   // ✅ FIXED
      priceTrial: parseFloat(priceTrial?.toString() || '0'),
      weeklyMenu: weeklyMenuValidation.data!
    };
    ...
    console.log('💾 Saving veg menu data');
    console.log('💰 Prices — Monthly:', menuData.priceMonthly, '| Weekly:', menuData.priceWeekly, '| Trial:', menuData.priceTrial);
    ...
    console.log('📊 Database entry:', {
      ...
      imageUrl: savedMenu?.imageUrl,
      imagePublicId: savedMenu?.imagePublicId,
      priceMonthly: savedMenu?.priceMonthly,
      priceWeekly: savedMenu?.priceWeekly,   // ✅ Log mein bhi show hoga
      priceTrial: savedMenu?.priceTrial
    });
...
// ✅ Create or update NON-VEG menu — priceWeekly BUG FIXED
export const createOrUpdateNonVegMenu = async (req: Request, res: Response): Promise<void> => {
  try {
    console.log('🚀 NON-VEG Menu Controller called');
    console.log('🌐 Using Cloudinary:', useCloudinary ? 'YES' : 'NO');
