...
// middleware/appMiddleware.ts - FIXED VERSION FOR RATE LIMIT ISSUES
import express, { Express, Request, Response, NextFunction } from 'express';
import helmet from 'helmet';
import rateLimit from 'express-rate-limit';
import compression from 'compression';
...
export const generalLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: process.env.NODE_ENV === 'production' ? 100 : 999999, // Unlimited for development
  message: {
    success: false,
    message: 'Too many requests from this IP, please try again later.',
    error: 'RATE_LIMIT_EXCEEDED'
...
export const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: process.env.NODE_ENV === 'production' ? 5 : 999999, // Unlimited for development
  message: {
    success: false,
    message: 'Too many authentication attempts, please try again later.',
    error: 'AUTH_RATE_LIMIT_EXCEEDED'
...
export const messageLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: process.env.NODE_ENV === 'production' ? 10 : 999999, // Unlimited for development
  message: {
    success: false,
    message: 'Too many messages submitted, please try again later.',
    error: 'MESSAGE_RATE_LIMIT_EXCEEDED'
...
export const uploadLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: process.env.NODE_ENV === 'production' ? 20 : 999999, // Unlimited for development
  message: {
    success: false,
    message: 'Too many upload attempts, please try again later.',
    error: 'UPLOAD_RATE_LIMIT_EXCEEDED'
...
export const configureMiddleware = (app: Express): void => {
  ...
  if (!isDevelopment) {
    console.log('✅ Applying rate limiting (PRODUCTION ONLY)...');
    app.use('/api/', generalLimiter);
    app.use('/api/auth', authLimiter);
    app.use('/api/messages', messageLimiter);
    app.use('/api/contact', messageLimiter);
    ...
    console.log('⚠️ Rate limiting COMPLETELY DISABLED for development');
  }
  ...
  app.get('/health', (req: Request, res: Response): void => {
    res.status(200).json({
      ...
      uptime: process.uptime(),
      environment: process.env.NODE_ENV || 'development',
      rateLimiting: !isDevelopment ? 'enabled' : 'disabled'
    });
  });
  ...
  console.log('✅ Security middleware configured successfully');
  console.log(`✅ Environment: ${isDevelopment ? 'DEVELOPMENT' : 'PRODUCTION'}`);
  console.log(`✅ Rate limiting: ${!isDevelopment ? 'ENABLED (Production)' : 'DISABLED (Development)'}`);
  console.log('✅ All other middleware configured');
};